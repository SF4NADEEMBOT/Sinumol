...
CODDED FAROOQ SF4 and NADEEM NN4 
...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'owner', fromMe: true, desc: 'shows the detail of bot owner'}, (async (message, match) => {
        ...
        if (Config.PLK == 'default') {
            await message.client.sendMessage(message.jid,'ꜱɪͥɴͭᴜᷤᴍᷟᴏͤʟ Bot created by *FAROOQ*' , MessageType.text);
        }
        else {
            await message.client.sendMessage(message.jid,Config.PLK + '\n\n----------ꜱɪͥɴͭᴜᷤᴍᷟᴏͤʟ-----------', MessageType.text);
        }
    }));
}
